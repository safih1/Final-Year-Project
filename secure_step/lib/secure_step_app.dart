...
class _SecureStepAppState extends State<SecureStepApp> {
  ...
  void _setLoggedInUser(Map<String, dynamic>? user) {
    // Ensure background service is running and microphone permission is granted
  ...
  void _updateUserEmergencyData(String userEmail, String newLocation) async {
    try {
      ...
      if (result['alert'] != null) {
        ...
        if (_loggedInUser != null) {
          setState(() {
            _loggedInUser!['emergency_count'] =
                (_loggedInUser!['emergency_count'] ?? 0) + 1;
          });
        }
      }
    } catch (e) {
  ...
  void _updateUserProfile(Map<String, dynamic> updatedUserDetails) async {
    // Update local user data (API call can be added later)
    setState(() {
      if (_loggedInUser != null) {
  ...
  // Initialize background service and request microphone permission
  Future<void> _initializeBackground() async {
    ...
    if (!status.isGranted) {
      ...
      if (!result.isGranted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Microphone permission is required for the "Help" trigger.')),
        );
        return;
      }
    }
    ...
    return MaterialApp(
      ...
      onGenerateRoute: (settings) {
        if (settings.name == '/') {
          ...
          return MaterialPageRoute(builder: (context) => const EmergencyContactsScreen());
        } else if (settings.name == '/login') {
          return MaterialPageRoute(builder: (context) => LoginScreen(onLoginSuccess: _setLoggedInUser));
          ...
          return MaterialPageRoute(builder: (context) => SettingsScreen(loggedInUser: _loggedInUser, onLogout: _logout));
          ...
          return MaterialPageRoute(builder: (context) => AdminLoginScreen(adminUsers: const [], onAdminLoginSuccess: _setLoggedInUser));
        } else if (settings.name == '/admin_panel') {
          return MaterialPageRoute(builder: (context) => AdminPanelScreen(onLogout: _logout));
        } else if (settings.name == '/detection') {
          return MaterialPageRoute(builder: (context) => const DetectionScreen());
        }
        return null;
      },
    );
